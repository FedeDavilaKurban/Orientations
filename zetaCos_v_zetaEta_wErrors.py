...
calculados metiendo ruido gaussiano en los cosenos.
Esto lo hago para simular posibles errores observacionales.

Calculo de error en Beta dado un error en cos(theta):

x = cos(theta)
dx = gauss(0, mean(cos)/10)

b = tan(arccos(x))
db = dtan(arccos(x)) dx = {sec[arccos(x)]}**2 dx = x**-2 dx 
"""
#%%
import sys
...
for rseed in rseeds:
    ...
    for k, (a, b, c) in enumerate(zip(aa,bb,cc)):
        ...
        beta.append( np.tan(np.arccos(cos[-1])) )

        # #Caluclating beta of the std of the measurement of the cosine
        # std_cos = abs(np.random.normal(0,.05,len(cos[-1]))) #STD of the gaussian noise 
        #                                         #is 10% of the mean
        ...
        eta_m_wErr, eta_std_wErr, eta_array_wErr = get_eta_bs_wErrors(beta[-1],cosErr=cosErr,Nbs=Nbs) 
...
ratio = np.array(eta)/np.array(eta_wErr)

#plt.axhline(1,color='k',ls=':')
ax.axhline(np.mean(ratio),color='k')
ax.axvline(eta_ran,color='k',ls='--')


ax.scatter(eta,ratio)#,'o--')
#ax.set_xticks([])
ax.set_xlabel(r'$\eta$')
ax.set_ylabel(r'$\eta / \eta_\mathrm{w/error}$')
ax.text(.1,.95,r'$\langle \eta / \eta_\mathrm{w/error} \rangle =$'+f'{np.mean(ratio):.4f}',\
                transform = ax.transAxes)
plt.savefig(f'../plots/zetaCos_v_zetaEta_wErrors/v2_ratio_N{Nran}_cosErr{cosErr}.jpg')
