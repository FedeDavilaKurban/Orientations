...
# S-Type
#############
vtype, label, fmt, c = ['s','S-Type','^-.','C00']
voids = readVoids(minrad=minradV,maxrad=maxradV,vtype=vtype)

zeta = []
zeta_std = []
for rmin,rmax in zip(r1,r2):
    filename = f'../data/zeta/zeta_minradV{minradV}_maxradV{maxradV}_rmin{rmin:.1f}_rmax{rmax:.1f}_sec{sec}_vtype{vtype}_jk.txt'
    zeta_jk = ascii.read(filename, names=['zeta_jk'])['zeta_jk']
    zeta.append( np.mean(zeta_jk) )
    zeta_std.append( np.sqrt((np.var(zeta_jk,ddof=1))))#/len(zeta_jk)) )

axs[0].errorbar(x,zeta,yerr=zeta_std,label=label,fmt=fmt,\
...
# R-Type
#############
vtype, label, fmt, c = ['r','R-Type','o-','C03']
voids = readVoids(minrad=minradV,maxrad=maxradV,vtype=vtype)
...
# All voids
#############
vtype, label, fmt, c = ['a','All Voids','x:','k']
voids = readVoids(minrad=minradV,maxrad=maxradV,vtype=vtype)
