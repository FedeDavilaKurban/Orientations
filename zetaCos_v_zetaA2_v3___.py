...
def plot_a2hist(a2_bs,a2,ax):
    ...
    for i in range(len(cc)):
        ...
        ax.text(.65,.06,f'{Nbs} bootstrap \n   resamplings',\
            color='k',fontsize=fs,transform = ax.transAxes)
...
ax5.set_xlabel(r'$\zeta_{\cos}=(\langle \cos\rangle -\langle \cos_{ran}\rangle)/\sigma_{cos}}$')
ax5.set_ylabel(r'$\zeta_{a_{1}}=(\langle a_{1}\rangle-\langle a_{1,ran}\rangle)/\sigma_{a_{1},ran}}$')
...
plt.savefig(f'../plots/zetaCos_v_zetaA2/a2_Nran{Nran}_Nbs{Nbs}_nseed{nseed}_v3.png')
plt.show()
# %%
