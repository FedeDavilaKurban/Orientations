...
for _ in range(len(voidsFinal)): 
	...
	plt.title('Void {}'.format(nv),size=12)
	plt.tight_layout()
	...
	for sec in [123,456]:
		...
		if sec==123: 
			...
			plt.text(0.4,-0.044,'N of {} Gxs: {}'.format(label,len(gals_h)))
		else: 
			ls,label=['g--','Low Spin']
			plt.text(0.4,-0.038,'N of {} Gxs: {}'.format(label,len(gals_h)))
		plt.step(x1,y1)
...
#	y1_slice3 = []
	...
	plt.legend()
	plt.text(0.4,-0.032,'N of Total Gxs: {}'.format(len(gals)))
	#plt.text(-.95,0.035,'x = {:.3} Mpc'.format(x/1000.))
	#plt.text(-.95,0.029,'y = {:.3} Mpc'.format(y/1000.))
	#plt.text(-.95,0.023,'z = {:.3} Mpc'.format(z/1000.))
	#plt.text(-.95,0.017,'r = {:.3} Mpc'.format(r/1000.))
	...
	plt.savefig('Void{}_Alignment.pdf'.format(nv))
