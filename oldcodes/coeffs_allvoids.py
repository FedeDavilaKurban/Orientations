"""
Escribe en la tabla 'pvalue...' los valores a1, a2, p1, p2 para cada void
...
pvalue1 = []
pvalue2 = []
a_ran = ascii.read('ran_coeffs_{0}-{1}-{2}.txt'.format(rmin,rmax,perc))
...
for nv in range(len(voidsFinal)):
	...
	pvalue1.append( float(len([x for x in a1_ran if x>a1[nv]])) / float(len(a1_ran)) )
	pvalue2.append( float(len([x for x in a2_ran if x<a2[nv]])) / float(len(a2_ran)) )

	print pvalue1[nv], pvalue2[nv]
	...
	#n1, bins, patches = ax1.hist(a1_ran,bins=50,normed=True,histtype='stepfilled')
	#n2, bins2, patches2 = ax2.hist(a2_ran,bins=50,normed=True,histtype='stepfilled')
...
col_p1 = Table.Column(pvalue1,'pvalue1')
col_p2 = Table.Column(pvalue2,'pvalue2')

col_a1 = Table.Column(a1,'a1')
col_a2 = Table.Column(a2,'a2')


pvalue = Table()
pvalue.add_columns([col_a1,col_a2,col_p1,col_p2])

ascii.write(pvalue,'pvalue_{0}-{1}-{2}.txt'.format(rmin,rmax,perc))
