...
for nv in range(len(voidsFinal)):
	...
	for i in range(len(gals)):
		...
		v = gals[i]['sp']
		cos.append( abs( np.dot(u,v)/(np.linalg.norm(u)*np.linalg.norm(v)) ) )
	...
	cos_h1.append( gals_h['cos'].data )

for i in range(len(vtype)-1):
     cos_h = np.append(cos_h1[i],cos_h1[i+1])
...
ax1.hist(cos_h,bins=15,normed=True,histtype='stepfilled',cumulative=False,alpha=30.)
...
fig.suptitle('Rmin, Rmax: '+str(rmin)+', '+str(rmax) )
