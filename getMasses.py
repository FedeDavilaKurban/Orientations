...
def readTNG_():
    ...
    gxs = Table(np.column_stack([pos[:,0],pos[:,1],pos[:,2]\
                ...
                names=['x','y','z','mass','spx','spy','spz','sp_n','gasmass','starmass']) 

    del mass,pos,spin,sp_n,masstype,gasmass,starmass
    ...
    for vtype in ['a','r','s']:
        ...
        for rmin,rmax in zip(r1,r2):
            ...
            for nv in nvs:
                ...
                mass.append(gals['mass'].data)
                gmass.append(gals['gasmass'].data)
                smass.append(gals['starmass'].data)


            mass = np.concatenate(mass)
            gmass = np.concatenate(gmass)
            smass = np.concatenate(smass)

            #print(rrv)
            
            ascii.write(np.column_stack([mass,gmass,smass]),\
                        '../data/mass/-1/mass_minradV{}_maxradV{}_rmin{:.1f}_rmax{:.1f}_sec{}_vtype{}.txt'\
                        .format(minradV,maxradV,rmin,rmax,sec,vtype),\
                        names=['mass','gmass','smass'],\
                        overwrite=True)
# %%
